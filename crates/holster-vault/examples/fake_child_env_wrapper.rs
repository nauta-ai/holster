...
const PASSWORD: &str = "fake-child-wrapper-password-2026";
const FAKE_VALUE: &str = "sk-test-fake-child-wrapper-2026";
const ENV_NAME: &str = "HOLSTER_FAKE_SMOKE_KEY";
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let vault_path = root.join("fake-child-wrapper-vault.db");
    let audit_path = root.join("fake-child-wrapper-audit.jsonl");
    let profile_path = root.join("fake-child-wrapper-agents.json");
    ...
    let key_id = vault
        .add_key(
            ...
            AddKeyInput {
                ...
                label: "fake-child-wrapper-smoke".to_string(),
                key_value: FAKE_VALUE.to_string(),
                project_tag: Some("fake-codex".to_string()),
                expires_at: None,
                notes: Some("fake child wrapper smoke only".to_string()),
            },
        )?
        .id;
    ...
    let secret = vault.fetch_key_for_agent(token, "codex", key_id, &profiles, &audit)?;

    let output = Command::new("/bin/sh")
        .arg("-c")
        .arg(format!(
            "test -n \"${ENV_NAME}\" && printf 'child_env_present\\n'"
        ))
    ...
    if !output.status.success() {
        return Err("child process did not receive fake env var".into());
    }
    let stdout = String::from_utf8(output.stdout)?;
    if stdout.trim() != "child_env_present" {
        return Err("child process printed unexpected output".into());
    }
    if stdout.contains(FAKE_VALUE) {
        return Err("child stdout leaked fake plaintext".into());
    }
    ...
    println!("fake_child_env_wrapper=ok");
